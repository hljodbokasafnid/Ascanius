  ...
  io.sockets.on('connection', function (socket) {
    ...
    socket.on('uploaded_convert_batch', async function (parent_name) {
      ...
      for (var book in books) {
        ...
        if (book_name !== undefined) {
          var preprocess = await spawn('python3', ['./scripts/preprocess.py', parent_name + '/' + books[book], book_name]);
          io.to(user_id).emit('newdata', preprocess.output.toString());
        }

        // See list of optional commands here: https://daisy.github.io/pipeline/modules/daisy202-to-epub3/
        var process = await spawn('dp2', ['daisy202-to-epub3', '--href', book_path + '/' + books[book] + '/ncc.html', '--output', output_path + 'output/' + parent_name, '--epub-filename', books[book] + '.epub', '-n', books[book]]);
