...
# REVIEW Metadata in head looks to be optional, therefore will not be included
...
def get_hms(time_float):
  seconds, milliseconds = str(time_float).split('.')
  minutes, seconds = divmod(int(seconds), 60)
  hours, minutes = divmod(int(minutes), 60)
  return "%02d:%02d:%02d.%s" % (hours, minutes, seconds, milliseconds.ljust(3, '0'))
...
def process_smil_files(files):
  ...
  for index, smil_file in enumerate(files):
    ...
    if smil_tag:
      ...
      print(index, total_elapsed_time_float)
